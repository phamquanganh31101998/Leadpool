<template>
    <v-content class="mt-4 pl-3 pr-3">
        <v-dialog v-model="gettingEmailContentDialog" hide-overlay persistent width="300">
        ...
        <v-dialog v-model="gettingEmailScheduleDetailDialog" hide-overlay persistent width="300">
            <v-card color="primary" dark>
                <v-card-text>
        ...
        <v-layout row wrap v-if="access">
            ...
            <v-flex xs6 sm8 md10 lg10 xl10 v-if="page=='manage'">
                <v-layout row>
                    <v-flex xs12 sm12 md12 lg12 xl12>
                        <span>
                            <v-btn class="mt-3" round color="#3E82F7" dark @click="startCreatingTemplate()">
                                <v-icon>add</v-icon> Tạo mẫu email mới
                            </v-btn>
                        </span>
                        <v-card class=" mt-3">
                            <v-card-text>
                                <v-data-table no-data-text="Không có kết quả nào phù hợp" :custom-sort="customSort"
                                    hide-actions :loading="loadingTable" rows-per-page-text="Hiển thị"
                                    :rows-per-page-items="[25,10,5, {text: 'Tất cả', value: -1}]" :headers="headers"
                                    :items="templateSelect">
                                    <template v-slot:items="props">
                                        <tr>
                                            <td> <a
                                                    @click="templateId = props.item.value, setChosenTemplate(props.item.value)">{{props.item.title}}</a>
                                            </td>
                                            ...
                                            <v-menu>
                                                ...
                                                <v-list>
                                                    <v-list-tile
                                                        @click="templateId = props.item.value, setChosenTemplate(props.item.value)">
                                                        <v-list-tile-content>Xem nội dung mẫu</v-list-tile-content>
                                                    </v-list-tile>
                                                    <v-list-tile
                                                        @click="createBtn = false, templateId = props.item.value, getHTMLAndCSS(props.item.value)">
                                ...
                                <div class="text-xs-center pt-2">
                                    <v-pagination @input="getEmailTemplate()" :total-visible="7" v-model="templatePage"
                                        :length="templateTotalPages"></v-pagination>
                                </div>
                            </v-card-text>
                        </v-card>
                    </v-flex>
                            ...
                            <v-layout row>
                                <v-flex xs12 sm12 md3 lg3 xl3>
                                    <v-card flat>
                                        <v-card-text>
                                            <span class="ml-4">
                                                <v-select prepend-icon="account_box" :items="allEmail"
                                                    v-model="createSchedule.from" label="Chọn tài khoản gửi email">
                                                </v-select>
                                            </span>
                                            <span class="ml-4">
                                                <v-text-field prepend-icon="title" v-model="createSchedule.title"
                                                    label="Tiêu đề"></v-text-field>
                                            </span>
                                            <span class="ml-4">
                                                <v-select :items="emailTemplateToSend"
                                                    v-model="createSchedule.chosenContentId" prepend-icon="textsms"
                                                    label="Chọn nội dung email"></v-select>
                                            </span>
                                            <span class="ml-4"><a v-if="createSchedule.chosenContentId != ''"
                                                    @click="templateId = createSchedule.chosenContentId, setChosenTemplate(templateId)">Xem
                                                    mẫu email </a></span>
                                            <br>
                                            <br>
                                            <v-divider :divider="divider"></v-divider>
                                            <v-select prepend-icon="people" :items="createSchedule.list"
                                                label="Chọn danh sách gửi"
                                                v-model="createSchedule.selectedListToSendSMS"></v-select>
                                            <v-divider :divider="divider"></v-divider>
                                            <span class="ml-4">
                                                <v-menu ref="menu1" v-model="createSchedule.menu1"
                                                    :close-on-content-click="false" lazy transition="scale-transition"
                                                    offset-y full-width max-width="290px" min-width="290px">
                                                    <template v-slot:activator="{ on }">
                                                        <v-text-field v-model="createSchedule.date"
                                                            label="Chọn ngày gửi" persistent-hint prepend-icon="event"
                                                            v-on="on">
                                                        </v-text-field>
                                                    </template>
                                                    <v-date-picker v-model="createSchedule.date" no-title
                                                        @input="createSchedule.menu1 = false"></v-date-picker>
                                                </v-menu>
                                            </span>
                                            <span class="ml-4">
                                                <v-select prepend-icon="access_time" label="Chọn giờ gửi"
                                                    v-model="createSchedule.time" :items="createSchedule.timeToChoose">
                                                </v-select>
                                            </span>
                                        </v-card-text>
                                    </v-card>
                                    <v-card flat>
                                        <v-card-actions>
                                            <v-btn dark block color="#3E82F7"
                                                :disabled="createSchedule.from == '' || createSchedule.title == '' || createSchedule.chosenContentId == '' || createSchedule.selectedListToSendSMS == ''"
                                                @click="sendEmail()">Đặt lịch gửi</v-btn>
                                        </v-card-actions>
                                    </v-card>
                                </v-flex>
                                <v-flex xs12 sm12 md9 lg9 xl9 class="ml-3">
                                    <v-card flat>
                                        ...
                                        <v-card-text>
                                            <v-data-table hide-actions :loading="loadingTable"
                                                rows-per-page-text="Hiển thị"
                                                :rows-per-page-items="[25,10,5, {text: 'Tất cả', value: -1}]" dense
                                                :headers="createSchedule.headers"
                                                :items="createSchedule.displayContacts" class="elevation-1"
                                                no-data-text="Chưa chọn danh sách ">
                                                <template v-slot:items="props">
                                                    <tr>
                                                        <!-- @change="checkChosenContact(props.item.contactId, props.item.chosen)" -->
                                                        <td>
                                                            <v-checkbox style="padding: 0px 0px 0px 0px; height: 30px;"
                                            ...
                                            <br>
                                            <v-pagination :total-visible="7" v-model="createSchedule.page"
                                                :length="createSchedule.pages" @input="getDisplayContactsOnOtherPage()">
                                            </v-pagination>
                                    ...
                                    <v-card flat class="mt-3">
                                        ...
                                        <v-card-text>
                                            ...
                                            <v-data-table hide-actions :loading="loadingTable"
                                                ...
                                                :rows-per-page-items="[25,10,5, {text: 'Tất cả', value: -1}]" dense
                                                :headers="createSchedule.headers"
                                                :items="createSchedule.additionalContacts" class="elevation-1">
                                                <template v-slot:items="props">
                                                    <tr>
                                                        <td>
                                                            <v-checkbox style="padding: 0px 0px 0px 0px; height: 30px;"
                                            ...
                                            <br>
                                            <v-pagination :total-visible="7" v-model="createSchedule.bonusPage"
                                                :length="createSchedule.bonusPages"
                                                @input="getAdditionalContactsOnOtherPage()"></v-pagination>
            ...
            <v-flex xs10 sm10 md10 lg10 xl10 v-if="page=='manageSchedule'">
                <v-layout row wrap>
                    <v-flex xs12 sm12 md12 lg12 xl12 class="mb-3 mt-3">
                        <v-btn round dark color="#3E82F7" @click="page='createSchedule'">
                            <v-icon>add</v-icon> Tạo lịch gửi mới
                        </v-btn>
                    </v-flex>
                    <v-flex xs12 sm12 md12 lg12 xl12>
                        <v-card width="100%">
                            <v-data-table :loading="loadingTable" rows-per-page-text="Hiển thị"
                                :rows-per-page-items="[25,10,5, {text: 'Tất cả', value: -1}]"
                                :headers="manageSchedule.headers" :items="manageSchedule.list">
                                <template v-slot:items="props">
                                    <tr>
                                        <!-- @change="checkChosenContact(props.item.contactId, props.item.chosen)" -->
                                        ...
                                        <td style="color: red" v-if="props.item.status == 'INACTIVE'">
                                            {{ returnStatus(props.item.status) }}</td>
                                        <td style="color: green" v-if="props.item.status == 'ACTIVE'">
                                            {{ returnStatus(props.item.status) }}</td>
                                        <td>{{props.item.createdBy}}</td>
                                        <v-menu offset-x>
                                            <template v-slot:activator="{ on }">
                                            ...
                                            <v-list>
                                                <v-list-tile
                                                    @click="templateId = props.item.templateId, setChosenTemplate(props.item.templateId)">
                                                ...
                                                <v-list-tile v-if="props.item.status == 'ACTIVE'"
                                                    @click="changeScheduleStatus(props.item.id, 'INACTIVE'), props.item.status = 'INACTIVE'">
                                                    <v-list-tile-content>Tắt lịch gửi</v-list-tile-content>
                                                </v-list-tile>
                                                <v-list-tile v-if="props.item.status == 'INACTIVE'"
                                                    @click="changeScheduleStatus(props.item.id, 'ACTIVE'), props.item.status = 'ACTIVE'">
                        ...
                        <v-card flat style="height: 300px; margin-top: 100px;">
                            <v-card-text style="height: 300px; background-color: #FDEDEE; border: 1px solid red;">
                                <v-card flat style="background-color: #FDEDEE; vertical-align: middle">
                                    ...
                                    <v-card-text>
                                        Bạn phải có quyền <span style="font-weight: bold">Xem tất cả</span> và <span
                                            style="font-weight: bold">Liên lạc tất cả</span> đối với Lead thì mới có thể
        ...
        <v-dialog v-model="manageSchedule.detail.dialog" width="60%" persistent>
            <v-card>
                ...
                <v-card-text>
                    <v-data-table :loading="loadingTable" rows-per-page-text="Hiển thị"
                        :rows-per-page-items="[25,10,5, {text: 'Tất cả', value: -1}]"
                        :headers="manageSchedule.detail.headers" :items="manageSchedule.detail.listEmail">
                        <template v-slot:items="props">
                            <tr>
                                <!-- @change="checkChosenContact(props.item.contactId, props.item.chosen)" -->
                                <td>{{ props.item.email}}</td>
                                <td>{{ returnTime(props.item.timeToSend) }} </td>
                                <td :style="returnStatusColor(props.item.status)">{{ returnStatus(props.item.status) }}
                                </td>
                            </tr>
                        </template>
                    </v-data-table>
                            ...
                            <div class="editor-canvas" style="height: 100%;">
                                <div id="gjs" style="height: 100%;">
                                    <div style="padding: 2% 5%">
                                        <img src="/adsplusbanner.jpg" alt="khuyenmai" style="width:100%;height:45%">
                                        <h3>Giải pháp quảng cáo Google Ads chuyên nghiệp</h3>
                                        <p>
                                            Chi phí quảng cáo Google Ads minh bạch với dịch vụ Quản lý tài khoản
                                            Triển khai gắn Goal đo lường chuyển đổi cho 100% Khách hàng - Nâng cao hiệu
                                            quả Quảng Cáo Google
                                            TOP 1 đối tác cao cấp Google tại Việt Nam
                                        </p>
                                        <h3>Quảng cáo Google Search Ads là gì?</h3>
                                        <p>Quảng cáo của bạn sẽ được hiển thị ở 4 vị trí đầu tiên và 3 vị trí cuối cùng
                                            trang tìm kiếm của Google và bạn chỉ trả tiền khi khách hàng nhấp vào để
                                            chuyển đến trang web của bạn.</p>
                                        <p>Google là trang web tìm kiếm thông tin được sử dụng nhiều nhất hiện nay. với
                                            vài từ khóa tìm kiếm, khách hàng có thể dễ dàng nhìn thấy và truy cập vào
                                            trang web của bạn. Với hàng triệu lượt truy cập mỗi ngày, thương hiệu của
                                            bạn sẽ nhanh chóng thu hút được sự chú ý và từ đó khách hàng sẽ cân nhắc và
                                            thực hiện hành động mua hàng.</p>
                                        <h3>AdsPlus sẽ giúp bạn khởi tạo chiến lược với cách tiếp cận bạn muốn</h3>
                                        <p>Dù bạn là doanh nghiệp mới thành lập, hay là công ty với nhiều năm hoạt động,
                                            hãy thay đổi chiến thuật quảng cáo ngay hôm nay để nâng tầm vị trí trong
                                            thương trường 4.0!</p>
                                        <br>
                                        <button
                                            style="background-color: #ff7f27;color:#fff;border-radius:20px; padding:2%">Đăng
                                            ký ngay</button>
                                    </div>
                                </div>
                            </div>
        ...
        <v-dialog v-model="create.dialog" width="30%" persistent>
            <v-card>
                ...
                <v-card-text>
                    ...
                    <span class="ml-4">
                        <v-text-field v-model="create.name"></v-text-field>
                    </span>
                </v-card-text>
                <v-divider :divider="divider"></v-divider>
                <v-card-actions>
        ...
        <v-dialog v-model="create.successfulDialog" @click:outside="create.successfulDialog = false"
        ...
        <v-dialog v-model="create.failDialog" @click:outside="create.failDialog = false"
            transition="dialog-bottom-transition" scrollable width="30%">
            <v-card tile>
        ...
        <v-dialog v-model="createSchedule.successfulDialog" @click:outside="createSchedule.successfulDialog = false"
            transition="dialog-bottom-transition" scrollable width="30%">
            <v-card tile>
                <v-toolbar card dark color="#00C853">
                    <v-toolbar-title>Thành công</v-toolbar-title>
        ...
        <v-dialog v-model="createSchedule.failDialog" @click:outside="createSchedule.failDialog = false"
            transition="dialog-bottom-transition" scrollable width="30%">
            <v-card tile>
                <v-toolbar card dark color="red">
                    <v-toolbar-title>Thất bại</v-toolbar-title>
        ...
        <v-dialog v-model="deleteEmailTemplateDialog.dialog" @click:outside="deleteEmailTemplateDialog.dialog = false"
            transition="dialog-bottom-transition" scrollable width="30%">
            <v-card tile>
                <v-toolbar card dark color="red">
    ...
    export default {
        ...
        methods: {
            ...
            getEmailTemplate() {
                ...
                this.templates = [];
                this.templateSelect = [];
                emailService.getEmailTemplate(this.idAccount, this.templatePage, this.sortBy, this.orderBy).then(
                    result => {
                        const {
                            dispatch
                        } = this.$store;
                    ...
                    result => {
                        ...
                        if (result.code == 'SUCCESS') {
                            for (let i = 1; i <= result.response.totalPage; i++) {
                                emailService.getEmailTemplate(this.idAccount, i, this.sortBy, this.orderBy).then(
                                    result => {
                ...
                this.editor = grapesjs.init({
                    ...
                    styleManager: {
                        ...
                        sectors: [{
                                ...
                                buildProps: ['width', 'height', 'padding-top', 'padding-bottom',
                            ...
                            },
                            {
                                name: 'Styling',
                                open: true,
                                buildProps: ['background-color', 'font-family', 'custom-prop', 'color',
                                    'font-size'
                                ],
                                properties: [{
                                        id: 'font-size',
            ...
            updateTemplate(id) {
                ...
                let content = `<!DOCTYPE html><html><head><style>` + css + `</style></head><body>` + html +
                    `</body></html>`
                let body = {
            ...
            getDisplayContactsOnOtherPage() {
                ...
                if (this.createSchedule.selectedListToSendSMS == 'all') {
                    contactService.getAllContact(this.idAccount, this.createSchedule.page).then(result => {
                        ...
                        if (result.code == 'SUCCESS') {
                            for (let k = 0; k < result.response.results.length; k++) {
                                result.response.results[k].chosen = !this.checkChosenContact(this.createSchedule
                                    .ignoreContact, result.response.results[k].contactId)
                    ...
                    listService.getContactByListId(this.idAccount, this.createSchedule.selectedListToSendSMS, this
                        .createSchedule.page).then(result => {
                        const {
                            dispatch
                        } = this.$store;
                        let time = moment();
            ...
            getAdditionalContactsOnOtherPage() {
                ...
                contactService.getAllContact(this.idAccount, this.createSchedule.bonusPage).then(result => {
                    ...
                    if (result.code == 'SUCCESS') {
                        for (let k = 0; k < result.response.results.length; k++) {
                            result.response.results[k].chosen = this.checkChosenContact(this.createSchedule
                                .bonusContact, result.response.results[k].contactId)
            ...
            getCurrentUser() {
                ...
                if ((role.includes('ROLE_SYSADMIN_SYSADMIN_ACCEPT')) || (role.includes(
                        'ROLE_CONTACT_COMMUNICATE_EVERYTHING') && role.includes("ROLE_CONTACT_VIEW_EVERYTHING"))) {
                    this.access = true;
                }
                if (this.access == true) {
                    this.getEmailTemplate();
