<template>
    <v-content class="mt-4 pl-2 pr-3">
        <v-dialog v-model="gettingTaskDialog" hide-overlay persistent width="300">
            ...
            <v-flex xs12 sm12 md2 lg2 xl2>
                <v-layout row>
                    ...
                    <v-flex xs12 md12 lg12 xl12>
                        <v-btn round block dark color="#3E82F7" @click="createTask = true">
                            <v-icon>add</v-icon> Tạo công việc mới
                        </v-btn>
                        <v-dialog v-model="createTask" persistent max-width="700px">
                            <v-card>
                                ...
                                <v-card-text>
                                    <newTask :allEmail="this.viewTask.allEmail"
                                        :searchedEmail="this.viewTask.searchedEmail" :idAccount="this.idAccount"
                                        @closeCreateTaskDialog="createTask = false" />
                                </v-card-text>
                            </v-card>
                        </v-dialog>
                    </v-flex>
            ...
            <v-flex xs6 sm8 md10 lg10 xl10>
                <v-card width="100%">
                    <v-layout row wrap>
                        <v-flex xs12 sm12 md12 lg12 xl12>
                            ...
                            <v-data-table :loading="loadingTable" :headers="headers" :items="displayTasks" hide-actions
                                no-data-text="Không có công việc nào">
                                <template v-slot:items="props">
                                    <td v-if="props.item.status == 'NOTCOMPLETED'">
                                        <v-tooltip top>
                                            <template v-slot:activator="{ on }">
                                                <v-btn small fab dark color="grey lighten-1"
                                                    @click="updateTask(props.item.taskId, props.item.contactId, 'status', 'COMPLETED')"
                                                    v-on="on">
                                                    <v-icon>done</v-icon>
                                                </v-btn>
                                            </template>
                                    ...
                                    <td v-if="props.item.status == 'COMPLETED'">
                                        <v-tooltip top>
                                            <template v-slot:activator="{ on }">
                                                <v-btn v-on="on" small fab dark color="success"
                                                    @click="updateTask(props.item.taskId, props.item.contactId, 'status', 'NOTCOMPLETED')">
                                                    <v-icon>done</v-icon>
                                                </v-btn>
                                            </template>
                ...
                <v-layout row wrap>
                    <v-flex offset-xs5 offset-sm5 offset-md5 offset-lg5 offset-xl5>
                        <v-pagination v-if="length > 0" v-model="pagination.page" :length="length"
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-layout wrap>
                        <v-flex xs12 sm12 md12 lg12 xl12>
                            <v-layout row>
                                <v-flex xs4 sm5 md6 lg6 xl6>
                                    <v-text-field label="Tên" v-model="viewTask.task.title"
                                        @change="updateTask(viewTask.task.taskId, 'a', 'title', viewTask.task.title)">
                                    </v-text-field>
                                </v-flex>
                                <v-flex xs4 sm4 md4 lg4 xl4>
                                    <v-menu ref="menu1" v-model="viewTask.task.menu1" :close-on-content-click="false"
                                        lazy transition="scale-transition" offset-y full-width max-width="290px"
                                        min-width="290px">
                                        <template v-slot:activator="{ on }">
                                            <v-text-field v-model="viewTask.task.dueDateDate" label="Ngày hết hạn"
                                                persistent-hint prepend-icon="event" readonly v-on="on">
                                            </v-text-field>
                                        </template>
                                        <v-date-picker v-model="viewTask.task.dueDateDate" no-title
                                            @change="updateTaskTime(viewTask.task.taskId, 'a', 'dueDate', viewTask.task.dueDateDate, viewTask.task.dueDateTime), viewTask.task.menu1 = false">
                                        </v-date-picker>
                                    </v-menu>
                                </v-flex>
                                <v-flex xs4 sm3 md2 lg2 xl2>
                                    <v-select label="Giờ" v-model="viewTask.task.dueDateTime"
                        ...
                        <v-flex xs12 sm12 md12 lg12 xl12>
                            <v-textarea class="mt-2" name="input" label="Ghi chú..." v-model="viewTask.task.note"
                                @change="updateTask(viewTask.task.taskId, 'a', 'note', viewTask.task.note)">
                            </v-textarea>
                        </v-flex>
                        <!-- <v-flex xs12 sm12 md12 lg12 xl12>
                            <v-layout row>
                        ...
                        <v-flex xs12 sm12 md12 lg12 xl12 class="mt-2">
                            ...
                            <v-layout row class="mt-2">
                                <v-flex>
                                    <p>Kiểu</p>
                                    <v-menu v-model="viewTask.task.typeMenu" :close-on-content-click="false" top
                                        offset-y>
                                        <template v-slot:activator="{ on }">
                                        ...
                                        <v-list>
                                            <v-list-tile
                                                @click="changeType(viewTask.task.taskId, 'To-do'), viewTask.task.typeMenu = false"
                                                style="backgroundColor: white;">
                                                <v-list-tile-title>Công việc</v-list-tile-title>
                                            </v-list-tile>
                                            <v-list-tile
                                                @click="changeType(viewTask.task.taskId, 'Email'), viewTask.task.typeMenu = false"
                                                style="backgroundColor: white;">
                                                <v-list-tile-title>Gửi Email</v-list-tile-title>
                                            </v-list-tile>
                                            <v-list-tile
                                                @click="changeType(viewTask.task.taskId, 'Call'), viewTask.task.typeMenu = false"
                                                style="backgroundColor: white;">
                                ...
                                <v-flex>
                                    <p>Giao cho</p>
                                    <v-menu v-model="viewTask.task.assignMenu" :close-on-content-click="false" offset-y
                                        max-width="300">
                                        <template v-slot:activator="{ on }">
                                            <a color="indigo" v-on="on">
                                        ...
                                        <v-card>
                                            <v-card-title>
                                                <v-layout row wrap>
                                                    ...
                                                    <br> -->
                                                    <v-flex xs12 sm12 md12 lg12 xl12>
                                                        <v-autocomplete :items="viewTask.searchedEmail"
                                                            v-model="viewTask.task.assignedTo"
                                                            @input="updateTask(viewTask.task.taskId, 'a', 'assignedTo', viewTask.task.assignedTo), viewTask.task.assignMenu = false">
                                                        </v-autocomplete>
                                    ...
                                    <v-layout row>
                                        <v-flex>
                                            <v-menu :close-on-content-click="false" top offset-y
                                                v-model="viewTask.task.reminderMenu">
                                                <template v-slot:activator="{ on }"
                                                    v-if="viewTask.task.emailReminderDate != ''">
                                                    <span>
                                                        <a color="indigo" v-on="on">
                                                ...
                                                <v-list>
                                                    ...
                                                    <v-menu :close-on-content-click="false" offset-y
                                                        v-model="viewTask.task.menu2">
                                                        <template v-slot:activator="{ on }">
                                                            <v-list-tile v-on="on"
                                                                @click="viewTask.task.emailReminderChoice = 'Chọn ngày'">
                                                                <v-list-tile-title>Chọn ngày</v-list-tile-title>
                                                            </v-list-tile>
                                                        </template>
                                                        <v-date-picker v-model="viewTask.task.emailReminderDate"
                                                            no-title
                                                    ...
                                                    <v-list-tile
                                                        @click="setNoReminder(viewTask.task.taskId), viewTask.task.emailReminderChoice = 'Không nhắc trước', viewTask.task.menu2 = false, viewTask.task.reminderMenu = false">
                                ...
                                <v-flex v-if="viewTask.task.emailReminderChoice!= 'Không nhắc trước'">
                                    <p>Thời gian</p>
                                    <v-menu :close-on-content-click="false" lazy v-model="viewTask.task.timeMenu"
                                        ...
                                        <v-select v-model="viewTask.task.emailReminderTime"
                                            :items="viewTask.timeToChoose"
        ...
        <v-dialog v-model="logoutDialog" @click:outside="failDialog = false" transition="dialog-bottom-transition"
            scrollable width="30%">
            <v-card tile>
                <v-toolbar card dark color="red">
                    <v-toolbar-title>Thất bại</v-toolbar-title>
        ...
        watch: {
            ...
            'viewTask.searchEmail'() {
                ...
                for (let i = 0; i < this.viewTask.allEmail.length; i++) {
                    if (this.viewTask.allEmail[i].name.toLowerCase().includes(this.viewTask.searchEmail
                    .toLowerCase())) {
                ...
                taskService.getTaskById(this.idAccount, id).then(result => {
                    ...
                    if (result.code == 'SUCCESS') {
                        ...
                        if (result.response.emailReminder != null) {
                            result.response.emailReminderDate = this.coverTimeDateOnly(result.response
                                .emailReminder)
                            result.response.emailReminderTime = this.coverTimeHourOnly(result.response
                                .emailReminder)
