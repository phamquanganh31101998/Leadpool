<template>
    <v-content class="mt-4 pr-3 pd-3 pl-3">
        <v-dialog v-model="gettingSMSScheduleDetailDialog" hide-overlay persistent width="300">
        ...
        <v-layout row v-if="access">
            ...
            <v-flex xs6 sm8 md10 lg10 xl10 class="pl-3 mt-3">
                                ...
                                <v-layout row wrap>
                                    <v-flex xs12 sm12 md3 lg3 xl3>
                                        <v-card flat>
                                            <v-card-text>
                                                <span class="ml-4">
                                                    <v-select prepend-icon="textsms" label="Chọn mẫu tin nhắn"
                                                        :items="template.currentTemplates"
                                                        v-model="send.chosenContentId"></v-select>
                                                </span>
                                                <!-- <p>Chiến dịch này còn {{send.remain}} tin nhắn</p> -->
                                                <v-divider :divider="divider"></v-divider>
                                                <v-textarea box readonly rows="4" label="Nội dung"
                                                    v-model="send.chosenContent"></v-textarea>
                                                <v-divider :divider="divider"></v-divider>
                                                <span class="ml-4">
                                                    <v-select prepend-icon="list" label="Chọn chiến dịch gửi tin nhắn"
                                                        :items="saveKey.list" v-model="send.chosenCampaign"></v-select>
                                                </span>
                                                <v-divider :divider="divider"></v-divider>
                                                <v-select prepend-icon="people" :items="send.list"
                                                    label="Chọn danh sách gửi" v-model="send.selectedListToSendSMS">
                                                </v-select>
                                                <v-divider :divider="divider"></v-divider>
                                                <span class="ml-4">
                                                    <v-menu ref="menu1" v-model="send.menu1"
                                                        :close-on-content-click="false" lazy
                                                        transition="scale-transition" offset-y full-width
                                                        max-width="290px" min-width="290px">
                                                        <template v-slot:activator="{ on }">
                                                            <v-text-field v-model="send.date" label="Chọn ngày gửi"
                                                                persistent-hint prepend-icon="event" v-on="on">
                                                            </v-text-field>
                                                        </template>
                                                        <v-date-picker v-model="send.date" no-title
                                                            @input="send.menu1 = false"></v-date-picker>
                                                    </v-menu>
                                                </span>
                                                <span class="ml-4">
                                                    <v-select prepend-icon="access_time" label="Chọn giờ gửi"
                                                        v-model="send.time" :items="send.timeToChoose"></v-select>
                                                </span>
                                            </v-card-text>
                                        </v-card>
                                        <v-card flat>
                                            <v-card-actions>
                                                <v-btn dark block color="#3E82F7"
                                                    :disabled="send.chosenContentId == '' || send.chosenCampaign == '' || send.selectedListToSendSMS =='' "
                                                    @click="sendSMS()">Đặt lịch gửi</v-btn>
                                            </v-card-actions>
                                        </v-card>
                                    </v-flex>
                                        ...
                                        <v-card flat>
                                            ...
                                            <v-card-text>
                                                ...
                                                <v-data-table hide-actions :loading="loadingTable"
                                                    ...
                                                    :headers="send.headers" :items="send.displayContacts"
                                                    class="elevation-1" no-data-text="Chưa chọn danh sách ">
                                                ...
                                                <br>
                                                <v-pagination :total-visible="7" v-model="send.page"
                                                    :length="send.pages" @input="getDisplayContactsOnOtherPage()">
                                                </v-pagination>
                                                <br>
                                            </v-card-text>
                                        </v-card>
                                        <v-card flat class="mt-3">
                                            ...
                                            <v-card-text>
                                                ...
                                                <v-data-table hide-actions :loading="loadingTable"
                                                    rows-per-page-text="Hiển thị"
                                                    :rows-per-page-items="[25,10,5, {text: 'Tất cả', value: -1}]" dense
                                                    :headers="send.headers" :items="send.additionalContacts"
                                                    class="elevation-1">
                                                    <template v-slot:items="props">
                                                        <tr>
                                                            <!-- @change="checkChosenContact(props.item.contactId, props.item.chosen)" -->
                                                ...
                                                <br>
                                                <v-pagination :total-visible="7" v-model="send.bonusPage"
                                                    :length="send.bonusPages"
                            ...
                            <v-flex xs3 sm3 md3 lg3 xl3>
                                <v-card style="height: 220px;">
                                    <v-card-text>
                                        ...
                                        <span class="ml-4">
                                            <v-select :items="saveKey.list" v-model="saveKey.selectedCampaignId"
                                                @input="getStatisticAndHistory()"></v-select>
                                        </span>
                                    </v-card-text>
                                    <v-divider class="mt-4" :divider="divider"></v-divider>
                                    <v-card-actions>
                                        <v-btn block color="#3E82F7" dark @click="saveKey.createCampaign = true">
                                            <v-icon>add</v-icon>Tạo chiến dịch mới
                                        </v-btn>
                                            ...
                                            <v-card>
                                                ...
                                                <v-card-text>
                                                    <v-form v-model="saveKey.createValid">
                                                        ...
                                                        <span class="ml-4">
                                                            <v-text-field :rules="saveKey.inputRules"
                                                                v-model="saveKey.createData.campaign"></v-text-field>
                                                        </span>
                                                        <span class="mt-4"><strong>Nhập Key </strong></span>
                                                        <span class="ml-4">
                                                            <v-text-field :rules="saveKey.inputRules"
                                                                v-model="saveKey.createData.key"></v-text-field>
                                                        </span>
                                                        <span class="mt-4"><strong>Nhập branch name </strong></span>
                                                        <span class="ml-4">
                                                            <v-text-field :rules="saveKey.inputRules"
                                                                v-model="saveKey.createData.name"></v-text-field>
                                                        </span>
                                                    </v-form>
                                                </v-card-text>
                                                <v-divider :divider="divider"></v-divider>
                                                <v-card-actions>
                                                    <v-btn flat color="primary" @click="createCampaign()"
                                                        :disabled="!saveKey.createValid">Tạo</v-btn>
                                                    <v-btn flat color="red" @click="saveKey.createCampaign = false">Đóng
                                                    </v-btn>
                                                </v-card-actions>
                                            </v-card>
                                        </v-dialog>
                                                ...
                                                <v-layout row>
                                                    <v-flex xs4 class="text-center">
                                                        ...
                                                        <v-layout row class="mt-4">
                                                            <div class="score"
                                                                style="background-color:#00adef; padding-top:12%">
                                                    ...
                                                    <v-flex xs4 class="text-center">
                                                        ...
                                                        <v-layout row class="mt-4">
                                                            <div class="score"
                                                                style="background-color:#00adef; padding-top:12%">
                                                                {{saveKey.selectedCampaignDetail.success + saveKey.selectedCampaignDetail.fail}}
                                                            </div>
                                                        </v-layout>
                                                    </v-flex>
                                                    <v-flex xs4 class="text-center">
                                                        ...
                                                        <v-layout row class="mt-4">
                                                            <div class="score" style="background-color: #00C853;">
                                                                <v-btn style="color: white; margin-left: -8px" flat
                                                                    @click="saveKey.showSmsError = false, saveKey.showSmsTemplate = false, saveKey.showSmsSuccess = true">
                                                                    <span
                                                                        style="font-size: 20px">{{saveKey.selectedCampaignDetail.success}}</span>
                                                                </v-btn>
                                                            </div>
                                                        </v-layout>
                                                    </v-flex>
                                                    <v-flex xs4 class="text-center">
                                                        ...
                                                        <v-layout row class="mt-4">
                                                            <div class="score" style="background-color:#EF5350;">
                                                                <v-btn style="color: white; margin-left: -8px" flat
                                                                    @click="saveKey.showSmsSuccess = false, saveKey.showSmsError = true, saveKey.showSmsTemplate = false">
                                                                    <span
                                                                        style="font-size: 20px">{{saveKey.selectedCampaignDetail.fail}}</span>
                                                                </v-btn>
                                                            </div>
                                                        </v-layout>
                                                    </v-flex>
                            ...
                            <v-flex xs12 sm12 md12 lg12 xl12>
                                <v-layout class="mt-3" wrap v-if="saveKey.showSmsSuccess">
                                    <v-data-table :loading="loadingTable" rows-per-page-text="Hiển thị"
                                        ...
                                        style="width: 100%;" class="elevation-1" no-data-text="Chưa có tin nhắn nào"
                                        :headers="saveKey.selectedCampaignHistory.successHeaders"
                                        :items="saveKey.selectedCampaignHistory.success">
                                        <template v-slot:items="props">
                                            <tr>
                                ...
                                <v-layout class="mt-3" wrap v-if="saveKey.showSmsError">
                                    <v-data-table :loading="loadingTable" rows-per-page-text="Hiển thị"
                                        :rows-per-page-items="[25,10,5, {text: 'Tất cả', value: -1}]"
                                        style="width: 100%;" class="elevation-1" no-data-text="Chưa có tin nhắn nào"
                                        :headers="saveKey.selectedCampaignHistory.failHeaders"
                                        :items="saveKey.selectedCampaignHistory.fail">
                                        <template v-slot:items="props">
                                            <tr>
                                                <td>{{props.item.phoneNumber}}</td>
                                                <td>{{props.item.time}}</td>
                                                <td style="color: red">
                                                    ...
                                                    <v-tooltip top>
                                                        <template v-slot:activator="{ on }">
                                                            <span>
                                                                <v-icon color="red" dark v-on="on">help</v-icon>
                                                            </span>
                                                        </template>
                    ...
                    <v-flex xs3 sm3 md3 lg3 xl3>
                        <v-card>
                            ...
                            <v-card-text>
                                ...
                                <span class="ml-4">
                                    <v-select :disabled="template.creatingTemplate" :items="template.currentTemplates"
                                        v-model="template.selectedTemplateId" @input="setTemplateContent()"></v-select>
                                </span>
                                <v-btn block dark color="#3E82F7" :disabled="template.creatingTemplate"
                                    @click="startCreatingTemplate()">
                                    <v-icon>add</v-icon>Tạo mẫu mới
                                </v-btn>
                            </v-card-text>
                            <v-divider class="" :divider="divider"></v-divider>
                            <v-card-text v-if="template.creatingTemplate">
                                <span><strong>Tên mẫu</strong></span>
                                <v-text-field :rules="template.nameRules" placeholder="Nhập tên mẫu"
                                    v-model="template.name"></v-text-field>
                            </v-card-text>
                        </v-card>
                    </v-flex>
                    <v-flex xs9 sm9 md9 lg9 xl9 class="ml-3">
                                ...
                                <template v-if="!template.creatingTemplate">
                                    <v-layout>
                                        <v-flex xs5 sm5 md5 lg5 xl5>
                                            <v-textarea label="Chỉnh sửa mẫu tin nhắn tại đây" rows="8" box
                                                counter="160" maxlength="160" v-model="template.selectedTemplateContent"
                                                @focus="template.updateTemplateView = true"></v-textarea>
                                        </v-flex>
                                        <v-flex xs5 sm5 md5 lg5 xl5 offset-xs1 offset-sm1 offset-md1 offset-xl1
                                            offset-lg1 v-if="template.updateTemplateView">
                                            <v-textarea box label="Nội dung hiển thị (không dấu)" rows="8" readonly
                                                :value="normalText(template.selectedTemplateContent)"></v-textarea>
                                        </v-flex>
                                    </v-layout>
                                    <v-btn color="primary" @click="updateTemplate()"
                                        :disabled="normalText(this.template.selectedTemplateContent) == template.oldContent">
                                        Lưu lại</v-btn>
                                </template>
                                <template v-else>
                                    <v-layout>
                                        <v-flex xs5 sm5 md5 lg5 xl5>
                                            <v-textarea rows="8" label="Nhập mẫu tin nhắn mới tại đây" counter="160"
                                                maxlength="160" box v-model="template.content"></v-textarea>
                                        </v-flex>
                                        <v-flex xs5 sm5 md5 lg5 xl5 offset-xs1 offset-sm1 offset-md1 offset-xl1
                                            offset-lg1>
                                            <v-textarea rows="8" label="Nội dung hiển thị" disabled counter="160" box
                                                maxlength="160" :value="normalText(template.content)"></v-textarea>
                                        </v-flex>
                                    </v-layout>
                                    <v-btn v-if="template.creatingTemplate"
                                        :disabled="template.content == '' || template.name == ''" color="primary"
                                        @click="createNewTemplate()">
                                        <v-icon>add</v-icon>Tạo mới
                                    </v-btn>
                                    <v-btn v-if="template.creatingTemplate" color="grey" dark
                                        @click="template.creatingTemplate = false">Quay lại</v-btn>
                                </template>
                            </v-card-text>
                        </v-card>
                    </v-flex>
                ...
                <v-layout row wrap v-if="page=='schedule'">
                    <v-flex xs12 sm12 md12 lg12 xl12 class="mb-4">
                        <v-btn dark round color="#3E82F7" @click="page = 'send'">
                    ...
                    <v-flex xs12 sm12 md12 lg12 xl12>
                        <v-card width="100%">
                            <v-data-table :loading="loadingTable" rows-per-page-text="Hiển thị"
                                :rows-per-page-items="[25,10,5, {text: 'Tất cả', value: -1}]"
                                :headers="schedule.headers" :items="schedule.list">
                                <template v-slot:items="props">
                                    <tr>
                                        <!-- @change="checkChosenContact(props.item.contactId, props.item.chosen)" -->
                                        <td> <a
                                                @click.stop="openScheduleDetailDialog(props.item.id)">{{ props.item.campaign}}</a>
                                        </td>
                                        <td>{{ props.item.time }} </td>
                                        ...
                                        <v-menu offset-x>
                                            ...
                                            <v-list>
                                                ...
                                                <v-list-tile v-if="props.item.status == 'ACTIVE'"
                                                    @click="changeScheduleStatus(props.item.number, 'INACTIVE')">
                                                    <v-list-tile-content>Tắt lịch gửi</v-list-tile-content>
                                                </v-list-tile>
                                                <v-list-tile v-if="props.item.status == 'INACTIVE'"
                                                    @click="changeScheduleStatus(props.item.number, 'ACTIVE')">
                    ...
                    <v-dialog v-model="schedule.detail.dialog" width="40%" persistent>
                        <v-card>
                            ...
                            <v-card-text>
                                ...
                                <v-data-table :loading="loadingTable" rows-per-page-text="Hiển thị"
                                    :rows-per-page-items="[25,10,5, {text: 'Tất cả', value: -1}]"
                                    :headers="schedule.detail.headers" :items="schedule.detail.listPhone">
                                    <template v-slot:items="props">
                                        <tr>
                                            <!-- @change="checkChosenContact(props.item.contactId, props.item.chosen)" -->
                                            <td>{{ props.item.phoneNumber}}</td>
                                            <td>{{ returnTime(props.item.timeToSend) }} </td>
                                            <td :style="returnStatusColor(props.item.status)">
                                                {{ returnStatus(props.item.status) }}</td>
                                        </tr>
                                    </template>
                                </v-data-table>
        ...
        methods: {
            ...
            getDisplayContactsOnOtherPage() {
                ...
                if (this.send.selectedListToSendSMS == 'all') {
                    contactService.getAllContact(this.idAccount, this.send.page).then(result => {
                        ...
                        if (result.code == 'SUCCESS') {
                            for (let k = 0; k < result.response.results.length; k++) {
                                result.response.results[k].chosen = !this.checkChosenContact(this.send
                    ...
                    listService.getContactByListId(this.idAccount, this.send.selectedListToSendSMS, this.send.page)
                        .then(result => {
                            const {
                                dispatch
                            } = this.$store;
            ...
            getAdditionalContactsOnOtherPage() {
                ...
                contactService.getAllContact(this.idAccount, this.send.bonusPage).then(result => {
                    ...
                    if (result.code == 'SUCCESS') {
                        for (let k = 0; k < result.response.results.length; k++) {
                            result.response.results[k].chosen = this.checkChosenContact(this.send.bonusContact,
                                result.response.results[k].contactId)
            ...
            getPhoneNumberToSendSMS() {
                ...
                contactToSend = [...this.getChosenContactFromListByListId(id), ...this.getChosenContactFromListByListId(
                    'additional')]
                let uniqueContact = [];
