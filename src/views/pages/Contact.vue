<template>
  <v-content class="mt-4 pl-3 pr-3">
    <v-layout row wrap>
      ...
      <v-flex xs12 sm12 md7 lg7 xl7>
        <v-layout row wrap>
          ...
          <v-flex xs11 sm10 md7 lg7 xl7>
            <v-text-field label="Nhập từ khóa rồi nhấn Enter để tìm kiếm" v-model="search"
              @keyup.enter="section = 'search', searchContact() " append-icon="search" single-line hide-details>
            </v-text-field>
          </v-flex>
          <v-flex xs6 sm6 md1 lg1 xl1>
            <v-btn round dark color="#3E82F7" @click="uploadFileDialog.dialog = true">Import</v-btn>
          </v-flex>
          <v-flex xs6 sm6 md2 lg2 xl2 class="pl-4">
            <v-btn round dark color="#3E82F7" @click="checkInfo = true">
              <v-icon>person_add</v-icon> Tạo Lead mới
            </v-btn>
          </v-flex>
        </v-layout>
      </v-flex>
    ...
    <v-layout row wrap>
      <v-flex xs12 sm4 md2 lg2 xl2>
              ...
              <template v-for="(orCondition, orIndex) in conditions">
                <v-card flat>
                          ...
                          <v-layout row>
                            <v-flex xs9 sm9 md9 lg9 xl9 class="pt-2">
                              <p v-if="andCondition.condition == 'IN'"><span
                                  style="font-weight: bold;">{{getPropertyName(andCondition.property)}}</span> có trong
                                <template v-for="val in andCondition.value">
                                  <v-chip>{{val}}</v-chip>
                                </template>
                              </p>
                              <p v-if="andCondition.condition == 'EQUAL'"><span
                                  style="font-weight: bold;">{{getPropertyName(andCondition.property)}}</span> là <span
                                  style="font-weight: bold;">{{andCondition.value}}</span></p>
                              <p v-if="andCondition.condition == 'LIKE'"><span
                                  style="font-weight: bold;">{{getPropertyName(andCondition.property)}}</span> chứa
                                <span style="font-weight: bold;">{{andCondition.value}}</span></p>
                              <p v-if="andCondition.condition == 'GREAT_THAN'"><span
                                  style="font-weight: bold;">{{getPropertyName(andCondition.property)}}</span> lớn hơn
                                <span style="font-weight: bold;">{{andCondition.value}}</span></p>
                              <p v-if="andCondition.condition == 'LESS_THAN'"><span
                                  style="font-weight: bold;">{{getPropertyName(andCondition.property)}}</span> nhỏ hơn
                                <span style="font-weight: bold;">{{andCondition.value}}</span></p>
                            </v-flex>
                  ...
                  <v-card-actions style="padding: 8px 8px; margins: 0px 0px">
                    <v-menu v-model="newCondition.menu" :close-on-content-click="false" :nudge-width="100" offset-x
                      max-width="400">
                      <template v-slot:activator="{ on }">
                        <v-btn class="blue" outline round style="color: blue;" v-on="on">
                          <v-icon>add</v-icon>Thêm điều kiện
                        </v-btn>
                      </template>
                        ...
                        <v-card-text>
                          <v-layout row wrap>
                            <v-flex xs6 sm6 md6 lg6 xl6>
                              <v-select :items="newCondition.contactProperties" label="Thuộc tính"
                                v-model="newCondition.chosenProperty"
                                @input="newCondition.chosenConstant = 'LIKE', newCondition.value = ''"></v-select>
                            </v-flex>
                            <br>
                            <v-flex xs6 sm6 md6 lg6 xl6>
                              <v-select
                            ...
                            <template v-if="newCondition.chosenProperty == 'lifecycle_stage'">
                              <template v-if="newCondition.chosenConstant == 'IN'">
                                <v-flex xs12 sm12 md12 lg12 xl12>
                                  <v-select label="Chọn giá trị" :items="lifecycleStages" multiple chips
                                    v-model="newCondition.chosenLifecycleStageValue"></v-select>
                                  <v-btn :disabled="newCondition.chosenLifecycleStageValue.length == 0" class="blue"
                              ...
                              <template v-else-if="newCondition.chosenConstant == 'LIKE'">
                                <v-flex xs12 sm12 md12 lg12 xl12>
                                  ...
                                  <v-btn :disabled="newCondition.value.length == 0" class="blue" outline round
                                    style="color: blue;"
                              ...
                              <template v-else>
                                <v-flex xs12 sm12 md12 lg12 xl12>
                                  <v-select :items="lifecycleStages" v-model="newCondition.chosenLifecycleStage"
                            ...
                            <template v-else-if="newCondition.chosenProperty == 'contact_owner'">
                              <template v-if="newCondition.chosenConstant == 'IN'">
                                <v-flex xs12 sm12 md12 lg12 xl12>
                                  <v-text-field label="Nhập các tài khoản cần tìm kiếm, phân tách nhau bằng dấu phẩy"
                            ...
                            <template v-else-if="newCondition.chosenProperty == 'phone'">
                              <template v-if="newCondition.chosenConstant == 'IN'">
                                <v-flex xs12 sm12 md12 lg12 xl12>
                                  ...
                                  <v-btn :disabled="!newCondition.checkValidInPhone" class="blue" outline round
                                    style="color: blue;"
                              ...
                              <template v-else>
                                <v-flex xs12 sm12 md12 lg12 xl12>
                                  <v-text-field type="number" label="Nhập từ khóa" v-model="newCondition.value">
                                  </v-text-field>
                                  <v-btn :disabled="newCondition.value.length == 0" class="blue" outline round
                                    style="color: blue;"
                            ...
                            <template v-else-if="newCondition.chosenProperty == 'email'">
                              <template v-if="newCondition.chosenConstant == 'IN'">
                                <v-flex xs12 sm12 md12 lg12 xl12>
                                  <v-text-field label="Nhập các email cần tìm kiếm, phân tách nhau bằng dấu phẩy"
                                    v-model="newCondition.value"></v-text-field>
                                  <v-btn :disabled="newCondition.value.length == 0" class="blue" outline round
                                    style="color: blue;"
                            ...
                            <template v-else-if="newCondition.chosenProperty == 'city'">
                              <template v-if="newCondition.chosenConstant == 'IN'">
                                <v-flex xs12 sm12 md12 lg12 xl12>
                                  <v-combobox @input="removeNotCity(newCondition.chosenCities)" label="Chọn giá trị"
                                    :items="cities" multiple v-model="newCondition.chosenCities"></v-combobox>
                                  <!-- <v-select label="Chọn giá trị" :items="cities" multiple chips v-model="newCondition.chosenCities"></v-select> -->
                                  <v-btn :disabled="newCondition.chosenCities.length == 0" class="blue" outline round
                                    style="color: blue;"
                              ...
                              <template v-else>
                                <v-flex xs12 sm12 md12 lg12 xl12>
                                  ...
                                  <v-combobox :items="cities" v-model="newCondition.chosenCity" label="Chọn giá trị">
                                  </v-combobox>
                                  <v-btn :disabled="!cities.includes(newCondition.chosenCity)" class="blue" outline
                            ...
                            <template v-else-if="newCondition.chosenProperty == 'bussiness'">
                              <template v-if="newCondition.chosenConstant == 'IN'">
                                <v-flex xs12 sm12 md12 lg12 xl12>
                                  <v-select label="Chọn giá trị" :items="allBussiness" multiple chips
                                    v-model="newCondition.chosenBussinesses"></v-select>
                                  <v-btn :disabled="newCondition.chosenBussinesses.length == 0" class="blue" outline
                                    round style="color: blue;"
                              ...
                              <template v-else>
                                <v-flex xs12 sm12 md12 lg12 xl12>
                                  <v-select :items="allBussiness" v-model="newCondition.chosenBussiness"
                                    label="Chọn giá trị"></v-select>
                                  <v-btn class="blue" outline round style="color: blue;"
                            ...
                            <template v-else-if="newCondition.chosenProperty == 'service'">
                              <template v-if="newCondition.chosenConstant == 'IN'">
                                <v-flex xs12 sm12 md12 lg12 xl12>
                                  <v-select label="Chọn giá trị" :items="allService" multiple chips
                                    v-model="newCondition.chosenServices"></v-select>
                                  <v-btn :disabled="newCondition.chosenServices.length == 0" class="blue" outline round
                                    style="color: blue;"
                              ...
                              <template v-else>
                                <v-flex xs12 sm12 md12 lg12 xl12>
                                  <v-select :items="allService" v-model="newCondition.value" label="Chọn giá trị">
                                  </v-select>
                                  <v-btn :disabled="newCondition.value.length == 0" class="blue" outline round
                                    style="color: blue;"
              ...
              <v-dialog width="30%" v-model="saveFilterDialog" v-if="filterId != ''" persistent>
                ...
                <v-card>
                  ...
                  <v-card-text>
                    ...
                    <span class="ml-4">
                      <v-text-field :rules="nameRules" v-model="filterName"></v-text-field>
                    </span>
                  </v-card-text>
                  <v-divider :divider="divider"></v-divider>
                  <v-card-actions>
                    <v-btn flat color="primary" @click="updateFilter(filterId), saveFilterDialog = false"
                      :disabled="filterName == ''">Cập nhật</v-btn>
                ...
                <v-card>
                  ...
                  <v-card-actions>
                    <v-btn flat color="primary"
                      @click="createFilter(filterName, conditions), createFilterDialog = false"
                      :disabled="filterName == ''">Tạo</v-btn>
        ...
        <template v-else>
          ...
          <br>
          <!-- first condition -->
          <v-menu v-model="createFirstCondition.firstConditionMenu" :close-on-content-click="false" :nudge-width="100"
            ...
            <v-card style="width: 100%;">
              <v-card-text>
                <v-layout row wrap>
                  <v-flex xs6 sm6 md6 lg6 xl6>
                    <v-select :items="createFirstCondition.contactProperties" label="Chọn thuộc tính"
                      v-model="createFirstCondition.chosenProperty"
                      @input="createFirstCondition.chosenConstant = 'LIKE', createFirstCondition.value = ''"></v-select>
                  </v-flex>
                  <br>
                  <v-flex xs6 sm6 md6 lg6 xl6>
                    <v-select
                  ...
                  <template v-if="createFirstCondition.chosenProperty == 'lifecycle_stage'">
                    <template v-if="createFirstCondition.chosenConstant == 'IN'">
                      <v-flex xs12 sm12 md12 lg12 xl12>
                        ...
                        <v-btn :disabled="createFirstCondition.chosenLifecycleStageValue.length == 0" class="blue"
                          outline round style="color: blue;"
                    ...
                    <template v-else-if="createFirstCondition.chosenConstant == 'LIKE'">
                      <v-flex xs12 sm12 md12 lg12 xl12>
                        ...
                        <v-btn :disabled="createFirstCondition.value.length == 0" class="blue" outline round
                          style="color: blue;"
                    ...
                    <template v-else>
                      <v-flex xs12 sm12 md12 lg12 xl12>
                        <v-select :items="lifecycleStages" v-model="createFirstCondition.chosenLifecycleStage"
                          label="Chọn giá trị"></v-select>
                        <v-btn class="blue" outline round style="color: blue;"
                  ...
                  <template v-else-if="createFirstCondition.chosenProperty == 'phone'">
                    <template v-if="createFirstCondition.chosenConstant == 'IN'">
                      <v-flex xs12 sm12 md12 lg12 xl12>
                        ...
                        <v-btn :disabled="!createFirstCondition.checkValidInPhone" class="blue" outline round
                          style="color: blue;"
                    ...
                    <template v-else>
                      <v-flex xs12 sm12 md12 lg12 xl12>
                        <v-text-field label="Nhập từ khóa" type="number" v-model="createFirstCondition.value">
                        </v-text-field>
                        <v-btn :disabled="createFirstCondition.value.length == 0" class="blue" outline round
                          style="color: blue;"
                          @click="addFirstCondition(createFirstCondition.chosenProperty, createFirstCondition.chosenConstant, createFirstCondition.value, false)">
                  ...
                  <template v-else-if="createFirstCondition.chosenProperty == 'city'">
                    <template v-if="createFirstCondition.chosenConstant == 'IN'">
                      <v-flex xs12 sm12 md12 lg12 xl12>
                        <v-combobox @input="removeNotCity(createFirstCondition.chosenCities)" :items="cities"
                          v-model="createFirstCondition.chosenCities" multiple></v-combobox>
                        <!-- <v-select label="Chọn giá trị" :items="cities" multiple chips v-model="createFirstCondition.chosenCities"></v-select> -->
                        <v-btn :disabled="createFirstCondition.chosenCities.length == 0" class="blue" outline round
                          style="color: blue;"
                    ...
                    <template v-else>
                      <v-flex xs12 sm12 md12 lg12 xl12>
                        ...
                        <v-combobox :items="cities" v-model="createFirstCondition.chosenCity" label="Chọn giá trị">
                        </v-combobox>
                        <v-btn :disabled="!cities.includes(createFirstCondition.chosenCity)" class="blue" outline round
                          style="color: blue;"
                  ...
                  <template v-else-if="createFirstCondition.chosenProperty == 'bussiness'">
                    <template v-if="createFirstCondition.chosenConstant == 'IN'">
                      <v-flex xs12 sm12 md12 lg12 xl12>
                        ...
                        <v-btn :disabled="createFirstCondition.chosenBussinesses.length == 0" class="blue" outline round
                          style="color: blue;"
                    ...
                    <template v-else>
                      <v-flex xs12 sm12 md12 lg12 xl12>
                        <v-select :items="allBussiness" v-model="createFirstCondition.chosenBussiness"
                          label="Chọn giá trị"></v-select>
                        <v-btn class="blue" outline round style="color: blue;"
                  ...
                  <template v-else-if="createFirstCondition.chosenProperty == 'service'">
                    <template v-if="createFirstCondition.chosenConstant == 'IN'">
                      <v-flex xs12 sm12 md12 lg12 xl12>
                        ...
                        <v-btn :disabled="createFirstCondition.chosenServices.length == 0" class="blue" outline round
                          style="color: blue;"
                    ...
                    <template v-else>
                      <v-flex xs12 sm12 md12 lg12 xl12>
                        <v-select :items="allService" v-model="createFirstCondition.value" label="Chọn giá trị">
                        </v-select>
                        <v-btn :disabled="createFirstCondition.value.length == 0" class="blue" outline round
                          style="color: blue;"
                          @click="addFirstCondition(createFirstCondition.chosenProperty, createFirstCondition.chosenConstant, createFirstCondition.value, false)">
      ...
      <v-flex xs12 sm8 md10 lg10 xl10>
            ...
            <tr>
              ...
              <v-menu>
                ...
                <v-list>
                  ...
                  <v-list-tile v-if="canDelete(props.item.contactOwner)"
                    @click="confirmDeleteContact(props.item.contactId)">
        ...
        <v-data-table v-else-if="section == 'filter'" :loading="loadingTable" style="width: 100%" :headers="headers"
          :items="contacts" hide-actions class="elevation-1" no-data-text="Không có kết quả nào phù hợp">
          <template v-slot:items="props">
            <tr>
              <!-- <td><router-link :to="takeLink(props.item.contactId)">{{ props.item.lastName }} {{ props.item.firstName }}</router-link></td> -->
              <td><a @click="$router.push(takeLink(props.item.contactId))">{{ props.item.fullname }}</a></td>
        ...
        <v-data-table v-else :loading="loadingTable" style="width: 100%" :headers="headersSearch" :items="contacts"
          hide-actions class="elevation-1" no-data-text="Không có kết quả nào phù hợp">
          <template v-slot:items="props">
            <tr>
              <!-- <td><router-link :to="takeLink(props.item.contactId)">{{ props.item.lastName }} {{ props.item.firstName }}</router-link></td> -->
              <td><a @click="$router.push(takeLink(props.item.contactId))">{{ props.item.fullname }}</a></td>
    ...
    <v-dialog v-model="deleteFilterDialog" @click:outside="deleteFilterDialog = false"
      transition="dialog-bottom-transition" scrollable width="30%">
      <v-card tile>
    ...
    <v-dialog v-model="deleteContactDialog.dialog" @click:outside="deleteContactDialog.dialog = false"
      transition="dialog-bottom-transition" scrollable width="30%">
      <v-card tile>
        <v-toolbar card dark color="red">
          <v-toolbar-title>Xóa?</v-toolbar-title>
        ...
        <v-card-actions>
          ...
          <v-btn flat color="primary" @click="deleteContactDialog.dialog = false, deleteContactDialog.id = ''">Quay lại
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-dialog v-model="failDialog" @click:outside="failDialog = false" transition="dialog-bottom-transition" scrollable
      width="30%">
      <v-card tile>
        <v-toolbar card dark color="red">
    ...
    <v-dialog v-model="createFailDialog" @click:outside="createFailDialog = false, createFailResponse = ''"
      transition="dialog-bottom-transition" scrollable width="30%">
      <v-card tile>
        <v-toolbar card dark color="red">
          <v-toolbar-title>Thất bại</v-toolbar-title>
    ...
    <v-dialog v-model="cannotDeleteDialog" @click:outside="cannotDeleteDialog = false"
      transition="dialog-bottom-transition" scrollable width="30%">
      <v-card tile>
        <v-toolbar card dark color="red">
          <v-toolbar-title>Thất bại</v-toolbar-title>
    ...
    <v-dialog v-model="uploadFileDialog.dialog" width="400"> 
      <v-card>
        ...
        <v-card-text style="padding: 0px 0px;">
          <v-layout row>
            <div class="container">
              <div>
                <label>
                  <h4 style="font-weight: bold;">Chọn file excel từ máy tính của bạn để thêm lead (Định dạng .xlsx,
                    .xlsm, ...)</h4>
                  <br>
                  <br>
                  ...
                  <br>
                  <br>
                  <a href="https://drive.google.com/uc?export=download&id=17GK6txShIoewWnthjD3tlNB3MvRHJfek"
                    download>Tải file mẫu tại đây</a>
        ...
        <v-alert :value="!checkFileExtension(uploadFileDialog.file.name) && uploadFileDialog.file.name != ''"
          type="error">
          File không đúng định dạng của excel
        </v-alert>
        <v-card-actions>
          <v-btn color="primary" :disabled="!checkFileExtension(uploadFileDialog.file.name)" flat @click="submitFile()">
            Tải lên</v-btn>
            ...
            <v-container grid-list-xl>
              <v-layout wrap>
                ...
                <v-flex xs12 md12 lg12 xl12 style="padding: 0px 16px;" class="mb-4">
                  ...
                  <v-combobox item-text="text" item-value="value" v-model="city" :items="cities" label="Chọn thành phố">
                  </v-combobox>
                </v-flex>
                <v-flex xs12 md12 lg12 xl12 style="padding: 0px 16px;">
                  ...
                  <v-layout row wrap>
                    <v-flex xs12 sm12 md12 lg12 xl12>
                      <v-select label="Ngành nghề" v-model="bussiness" :items="allBussiness"
                        @change="changeBussiness(bussiness)"></v-select>
                    </v-flex>
                    <!-- <v-flex xs4 sm4 md4 lg4 xl4>
                              <v-checkbox v-model="isOtherBussiness" label="Ngành nghề khác:"></v-checkbox>
                            </v-flex> -->
                    <v-flex xs12 sm12 md12 lg12 xl12>
                      <v-text-field v-if="isOtherBussiness" v-model="otherBussinessValue"
                        placeholder="Nhập ngành nghề khác"></v-text-field>
                    </v-flex>
                  </v-layout>
                </v-flex>
              </v-layout>
  ...
  export default {
    ...
    data: () => ({
      ...
      allBussiness: ['Giáo dục (Trường ĐH, cao đẳng, TT ngoại ngữ',
        ...
        'Bất động sản',
        'Nội thất', 'Thương mại điện tử', 'Mỹ phẩm', 'Du học/ Định cư', 'Làm đẹp (Spa, salon, thẩm mỹ viện,...)',
      ...
      uploadFileDialog: {
        ...
        extension: ['xlsx', 'xlsm', 'xlsb', 'xltx',
          'xltm', 'xls', 'xlt', 'xml',
          'xlam', 'xla', 'xlr', 'csv', 'dif', 'ods', 'ots', 'tsv'
        ]
      }
    ...
    methods: {
      ...
      submitFile() {
        ...
        axios.post(link,
            ...
            if (result.data.code == 'SUCCESS') {
              ...
              this.uploadResult.SUCCESS.description = this.checkString(result.data.response.SUCCESS.description)
                .split(" ");
              this.uploadResult.DUPLICATE.description = this.checkString(result.data.response.DUPLICATE.description)
                .split(" ");
      ...
      getAllContact() {
        ...
        this.contacts = []
        this.allContacts = [];
        contacts.getAllContact(this.idUser, this.page, this.sortContact.property, this.sortContact.order).then(
          result => {
            const {
              dispatch
            } = this.$store;
            ...
            if (result.code == 'SUCCESS') {
              for (let i = 0; i < result.response.results.length; i++) {
                result.response.results[i].fullname = this.checkString(result.response.results[i].lastName) + ' ' +
                  this.checkString(result.response.results[i].firstName)
              }
      ...
      getMyContact() {
        ...
        this.contacts = []
        this.allContacts = [];
        contacts.getMyContact(this.idUser, this.page, this.sortContact.property, this.sortContact.order).then(
          result => {
            const {
              dispatch
            } = this.$store;
