            ...
            <v-flex xs12 sm12 md3 lg3 xl3 class="mr-3" style=" border-right: 1px solid #DCDCDC;">
                <div style="height: 100%;">
                    ...
                    <div class="fullHeight pr-3" style="background-color: #FAFAFA">
                        ...
                        <v-btn block small v-if="topicPage < allTopicPage" @click="getOlderTopic()">Xem các cuộc trò
                            chuyện cũ hơn</v-btn>
                    </div>
            ...
            <v-flex xs12 sm12 md9 lg9 xl9>
                <div>
                    ...
                    <div style="height: 100%; width: 100%">
                        <div>
                            <v-btn small block color="#3E82F7" dark @click="getOlderHistory(topic)"
                                v-if="chatPage < allChatPage">Xem các tin nhắn cũ hơn</v-btn>
                        </div>
                        <div class="messengerBox" id="boxContainChat">
                            <template v-for="item in chatHistory">
                                <v-layout align-center justify-start row :key="item.chatMessageId"
                                    v-if="item.isCustomer == true || item.isCustomer == null">
                                    <v-tooltip bottom>
                                        <template v-slot:activator="{ on }">
                                            <v-avatar v-on="on" :tile="tile" size="20" color="blue-grey"
                                ...
                                <v-layout align-center justify-end row :key="item.chatMessageId"
                        ...
                        <div class="stickToBottom">
                            <v-text-field append-icon="send" @keyup.enter="sendMessage()" @click:append="sendMessage()"
                                v-if="topic != ''" v-model="chatMess" placeholder="Nhập tin nhắn tại đây">
                            </v-text-field>
                        </div>
                    </div>
                </div>
                    ...
                    if (this.topic == this.topicChange) {
                        ...
                        if (this.newMessage.name != this.currentUser.displayName) {
                            ...
                            this.$store.dispatch('alert/success',
                                `${this.newMessage.name} đã gửi một tin nhắn mới tại cuộc trò chuyện ${this.decodeStr(this.topicChange)} (${this.coverTimeHourOnly(moment())})`
                                )
                        }
                ...
                chatAPI.getTopicByName(this.idAccount, topicName).then(result => {
                    ...
                    if (result.code == 'SUCCESS') {
                        ...
                        dispatch('alert/success',
                            `Bạn có 1 cuộc trò chuyện mới (${this.coverTimeDetail(time)})`)
                    } else {
                        dispatch('alert/error', `${result.message} (${this.coverTimeDetail(time)})`)
                    }
                })
