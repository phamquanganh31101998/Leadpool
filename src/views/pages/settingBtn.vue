<template>
    <v-content>
                    ...
                    <v-layout row class="pl-5">
                        <v-flex lg4 xl4>
                            ...
                            <div :style="styleBtn" v-if="xy">
                                <v-btn v-show="call" fab :dark="dark" :small="small" :large="large"
                                    :color="color">
                                    <v-icon>phone_in_talk</v-icon>
                                </v-btn>
                                <v-btn v-show="form" @click="showForDialog = true" fab
                                    :dark="dark" :small="small" :large="large" :color="colorForm">
                                    <v-icon>email</v-icon>
                                </v-btn>
                            </div>
                            <div :style="styleBtn" v-else>
                                <v-btn v-show="form" @click="showForDialog = true"
                                    fab :dark="dark" :small="small" :large="large" :color="colorForm">
                                    <v-icon>email</v-icon>
                                </v-btn>
                                <br />
                                <v-btn v-show="call" fab :dark="dark" :small="small" :large="large"
        ...
        <v-dialog v-model="showForDialog" max-width="400">
            <v-card>
                <v-card-title :background-color="colorForm">{{nameForm}}</v-card-title>
                ...
                <v-card-actions>
                    ...
                    <v-flex class="text-xs-right">
                        <v-btn color="green darken-1" text @click="showForDialog = false">Xác nhận</v-btn>
                    </v-flex>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        methods: {
            ...
            createGbtn() {
                ...
                for (let i = 0; i < this.input.length; i++) {
                    this.properties.push(this.input[i].value)
                }
                let form = {
                ...
                leadhubService.createGbtn(this.idAccount, btn).then(result => {
                    this.alertError(result.response)
                })
            }
        },
        components: {
